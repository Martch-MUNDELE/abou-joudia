...
export default function PanierPage() {
  ...
  if (items.length === 0) return (
    <div style={{ textAlign: 'center', padding: '80px 20px' }}>
      ...
      <p style={{ color: '#7A6E58', marginBottom: 28, fontSize: 13 }}>Ajoutez des produits depuis notre catalogue</p>
    ...
    <div style={{ maxWidth: 520, margin: '0 auto', paddingBottom: 100 }}>
          ...
          {stepLabels.map((label, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', flex: i < 2 ? 1 : 'none' }}>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 5 }}>
                <div style={{ width: 30, height: 30, borderRadius: '50%', background: stepIndex >= i ? 'linear-gradient(135deg,#F5C842,#FF6B20)' : 'rgba(255,255,255,0.05)', border: stepIndex >= i ? 'none' : '1.5px solid rgba(232,160,32,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 800, color: stepIndex >= i ? '#0A0804' : '#7A6E58', transition: 'all 0.3s', fontFamily: 'DM Sans, sans-serif' }}>
                  {stepIndex > i ? '✓' : i + 1}
                </div>
                <div style={{ fontSize: 11, fontWeight: 600, color: stepIndex >= i ? '#E8A020' : '#7A6E58', letterSpacing: '0.3px', textTransform: 'none' as const, whiteSpace: 'nowrap' as const }}>{label}</div>
              </div>
      ...
      {step === 'cart' && (
        <div style={{ padding: '0 20px' }}>
          ...
          {/* Livraison gratuite */}
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 0', borderBottom: '1px solid rgba(232,160,32,0.06)', fontSize: 12, color: '#7A6E58' }}>
            <span>Livraison</span>
      ...
      {step === 'info' && (
        <div style={{ padding: '0 20px', display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div>
            <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: '#7A6E58', textTransform: 'uppercase' as const, letterSpacing: '0.8px', marginBottom: 8 }}>Nom complet <span style={{ color: '#FF6B20' }}>*</span></label>
          ...
          <div>
            <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: '#7A6E58', textTransform: 'uppercase' as const, letterSpacing: '0.8px', marginBottom: 8 }}>Téléphone <span style={{ color: '#FF6B20' }}>*</span></label>
            <div style={{ display: 'flex', gap: 8 }}><PhoneInput value={form.phone} initialValue={form.phone} onChange={v => updateForm(f => ({ ...f, phone: v }))} /></div>
          </div>
          <div>
            <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: '#7A6E58', textTransform: 'uppercase' as const, letterSpacing: '0.8px', marginBottom: 8 }}>Adresse de livraison <span style={{ color: '#FF6B20' }}>*</span></label>
          ...
          <div>
            <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: '#7A6E58', textTransform: 'uppercase' as const, letterSpacing: '0.8px', marginBottom: 8 }}>Note (optionnel)</label>
          ...
          <div style={{ background: 'rgba(232,160,32,0.04)', border: '1px solid rgba(232,160,32,0.12)', borderRadius: 12, padding: '14px 16px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }} onClick={() => updateForm(f => ({ ...f, wantFacture: !f.wantFacture }))}>
              ...
              <div>
                ...
                <div style={{ fontSize: 11, color: '#7A6E58', marginTop: 2 }}>Envoyée par email après commande</div>
              </div>
            </div>
            {form.wantFacture && (
      ...
      {step === 'slot' && (
        <div style={{ padding: '0 20px' }}>
          ...
          <p style={{ textAlign: 'center', color: '#7A6E58', fontSize: 11, marginTop: 16, letterSpacing: '0.3px' }}>Paiement en cash à la livraison uniquement</p>
        </div>
      )}
        ...
        <div style={{ maxWidth: 520, margin: '0 auto' }}>
          ...
          {step === 'cart' && (
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              <span style={{ fontSize: 13, color: '#7A6E58', fontWeight: 500 }}>{items.reduce((acc, i) => acc + i.quantity, 0)} article{items.reduce((acc, i) => acc + i.quantity, 0) > 1 ? 's' : ''}</span>
