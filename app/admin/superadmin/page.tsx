...
const STATUS_COLORS: Record<string, { bg: string; color: string }> = {
  ...
  deleted:  { bg: 'rgba(122,110,88,0.1)',  color: '#7A6E58' },
}
    ...
    <div style={{ maxWidth: 800, margin: '0 auto', padding: '24px 16px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          ...
          <div style={{ fontSize: 11, color: '#7A6E58', marginTop: 4 }}>Accès restreint · {currentUser?.email}</div>
        </div>
      ...
      {msg && (
        <div style={{ background: 'rgba(232,160,32,0.08)', border: '1px solid rgba(232,160,32,0.2)', borderRadius: 12, padding: '12px 16px', marginBottom: 16, fontSize: 13, color: '#F5C842', fontFamily: 'DM Sans, sans-serif', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span>{msg}</span>
          <button onClick={() => setMsg('')} style={{ background: 'none', border: 'none', color: '#7A6E58', cursor: 'pointer', fontSize: 16 }}>×</button>
        </div>
      )}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200, padding: 16 }}>
          <div style={{ background: '#131009', border: '1px solid rgba(232,160,32,0.2)', borderRadius: 20, width: '100%', maxWidth: 440, padding: 28 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              ...
              <button onClick={() => setShowNew(false)} style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 50, width: 32, height: 32, color: '#7A6E58', cursor: 'pointer', fontSize: 16 }}>×</button>
            </div>

            <div style={{ marginBottom: 14 }}>
              <label style={{ fontSize: 11, fontWeight: 700, color: '#7A6E58', display: 'block', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.8px' }}>Email</label>
            ...
            <div style={{ marginBottom: 16 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                <label style={{ fontSize: 11, fontWeight: 700, color: '#7A6E58', textTransform: 'uppercase', letterSpacing: '0.8px' }}>Mot de passe</label>
                <button onClick={() => setAutoGen(v => !v)} style={{ fontSize: 10, padding: '3px 10px', borderRadius: 50, border: '1px solid rgba(232,160,32,0.2)', background: autoGen ? 'rgba(232,160,32,0.1)' : 'transparent', color: autoGen ? '#E8A020' : '#7A6E58', cursor: 'pointer', fontWeight: 600 }}>
                  {autoGen ? 'Auto' : 'Manuel'}
                </button>
                {autoGen && <button onClick={() => setNewPassword(generatePassword())} style={{ fontSize: 10, padding: '3px 10px', borderRadius: 50, border: '1px solid rgba(232,160,32,0.1)', background: 'transparent', color: '#7A6E58', cursor: 'pointer' }}>↻ Générer</button>}
              </div>
              {autoGen ? (
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setShowNew(false)} style={{ flex: 1, padding: '11px', borderRadius: 50, border: '1px solid rgba(255,255,255,0.08)', background: 'transparent', color: '#7A6E58', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif', fontWeight: 700, fontSize: 13 }}>Annuler</button>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
        {(['admins', 'logs'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)} style={{ padding: '7px 16px', borderRadius: 50, border: '1px solid', borderColor: tab === t ? 'rgba(232,160,32,0.4)' : 'rgba(232,160,32,0.12)', background: tab === t ? 'rgba(232,160,32,0.12)' : 'transparent', color: tab === t ? '#E8A020' : '#7A6E58', cursor: 'pointer', fontSize: 12, fontWeight: 600, fontFamily: 'DM Sans, sans-serif' }}>
              ...
              <div key={admin.id} style={{ background: '#131009', border: '1px solid rgba(232,160,32,0.1)', borderRadius: 14, padding: '16px 18px' }}>
                ...
                {pwd && admin.role !== 'superadmin' && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10, padding: '8px 12px', background: 'rgba(232,160,32,0.04)', border: '1px solid rgba(232,160,32,0.1)', borderRadius: 8 }}>
                    <span style={{ fontSize: 11, color: '#7A6E58' }}>Mot de passe :</span>
                    <span style={{ fontFamily: 'monospace', fontSize: 12, color: showPwd[admin.id] ? '#F5C842' : '#3A3020', letterSpacing: 1 }}>
                      {showPwd[admin.id] ? pwd : '••••••••••••'}
                    </span>
                    <button onClick={() => setShowPwd(p => ({ ...p, [admin.id]: !p[admin.id] }))} style={{ fontSize: 10, padding: '2px 8px', borderRadius: 50, border: '1px solid rgba(232,160,32,0.15)', background: 'transparent', color: '#7A6E58', cursor: 'pointer' }}>
                ...
                {admin.role !== 'superadmin' && (
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    ...
                    <button onClick={() => deleteAdmin(admin)} style={{ padding: '5px 12px', borderRadius: 50, border: '1px solid rgba(255,107,107,0.15)', background: 'transparent', color: '#7A6E58', fontSize: 11, fontWeight: 600, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>Supprimer</button>
                  </div>
                )}
              </div>
            )
      ...
      {tab === 'logs' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {logs.map(log => (
            <div key={log.id} style={{ background: '#131009', border: '1px solid rgba(232,160,32,0.08)', borderRadius: 12, padding: '12px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div>
                ...
                {log.details && <div style={{ fontSize: 11, color: '#7A6E58', marginTop: 4 }}>{log.details}</div>}
              </div>
          ...
          {logs.length === 0 && <div style={{ textAlign: 'center', color: '#7A6E58', padding: '40px 0', fontSize: 14 }}>Aucune action enregistrée</div>}
        </div>
      )}
    </div>
  )
