...
const STATUS_COLORS: Record<string, { bg: string; color: string; border: string }> = {
  ...
  livrée:         { bg: 'rgba(122,110,88,0.1)',   color: '#7A6E58', border: 'rgba(122,110,88,0.2)'  },
    ...
    <div style={{ maxWidth: 720, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', gap: 8, overflowX: 'auto', marginBottom: 24, paddingBottom: 4 }}>
        <button
          ...
          style={{ flexShrink: 0, padding: '6px 14px', borderRadius: 50, border: '1px solid', borderColor: filter === 'all' ? 'rgba(232,160,32,0.4)' : 'rgba(232,160,32,0.12)', background: filter === 'all' ? 'rgba(232,160,32,0.12)' : 'transparent', color: filter === 'all' ? '#E8A020' : '#7A6E58', cursor: 'pointer', fontSize: 12, fontWeight: 600, fontFamily: 'DM Sans, sans-serif' }}
        >
          Toutes
        </button>
        {STATUSES.map(s => {
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        {orders.length === 0 && (
          <div style={{ textAlign: 'center', color: '#7A6E58', padding: '40px 0', fontSize: 14 }}>Aucune commande</div>
        )}
        {orders.map(order => {
            ...
            <div key={order.id} style={{ background: '#131009', border: '1px solid rgba(232,160,32,0.1)', borderRadius: 16, padding: '18px 20px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                <div>
                  ...
                  <div style={{ fontSize: 11, color: '#7A6E58', marginTop: 3 }}>{order.customer_phone} · {order.customer_address?.slice(0, 40)}{order.customer_address?.length > 40 ? '…' : ''}</div>
